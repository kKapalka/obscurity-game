...
public class DoubleAttack : Weapon
{
	...
	public override void HighlightSelection (GameObject go)
	{
		...
		marker.transform.localScale = new Vector3 (sc.x, sc.y * 8, sc.z)*Constants.boardLossyScale.x;
		markers.Add (marker);
		...
		marker.transform.localScale = new Vector3 (sc.x * 8, sc.y, sc.z) * Constants.boardLossyScale.x;
		markers.Add (marker);
